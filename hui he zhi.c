...
void main()
{
    ...
    {   printf("\n");
        printf("白魔法师 ·白昼的聆听者·\natk:300\ndef:100\nhp:800\n");
        printf("技能组：\n1.救疗[核心技能]：回复指定队友相当于自身atk 300%%的hp\n2.医治：为己方全体回复相当于自身atk 80%%的hp\n3.法令：对己方全体回复相当于自身atk 200%%的hp，对对方全体造成相当于自身atk 1000%%的伤害，每3回合只能使用一次\n");
        printf("是否选择白魔法师?(Y)\n");
        getchar();
        scanf("%c",&chose);
        if(chose=='Y')
...
{   
    ...
    int bm_chose,bm_time,*bm_t=&bm_time;                                                                   
    printf("确认你的小队\n");
    for(i=0;i<3;i++)
    { 
        switch (player[i])
   ...
   while(1)
    ...
    { 
        ...
        case 1:printf("\n战士\n");zhanshi(p1,t_zs_1,t_zs_2,zs_atk_zz,zs_def_zz);    //战士出手
            ...
            if(zs_atk_rank!=0)
            {
                enemy_hp-=zs_hit-enemy_def;                                         //战士伤害判定
        ...
        {
            enemy_hp-=hm_hit-enemy_def;                                             //黑魔伤害判定
        ...
        if(bm_atk_2!=0)
        {
            ...
            enemy_hp-=bm_hit-enemy_def;                                             //白魔伤害判定
            printf("白魔法师造成了%d的伤害\n",(int)(bm_hit-enemy_def));
        }
            break;
        case 4:printf("\n吟游诗人\n");
            break;
