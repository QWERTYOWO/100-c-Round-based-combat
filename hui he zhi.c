...
int shiren(int *t,float *sr_buff,float *sr_debuff,float *sr_atk,int *sr_use)
{
    ...
    case 2:*sr_debuff=0.5;//获得削弱倍率
        printf("且看苍穹之上，尼德霍格，禁城龙诗\n");
        break;
    case 3:*sr_atk+=0.2;//逐渐提高伤害倍率
            *sr_use=1;//判定是不是用技能
        printf("且看帝国之领，阿拉米格，红莲奔袭\n");
        break;
    }
    return 0;
...
{   
    ...
    float zs_hp=2000,zs_def=500,zs_atk=100,zs_hit;                                  //战士基础属性变量
    float bm_hp=800,bm_def=100,bm_atk=300,find_bm=1,bm_hit;                                   //白魔基础属性及仇恨变量
    ...
    int sr_cd,*sr_cd_zz=&sr_cd,sr_use=0,*sr_use_zz=&sr_use,sr_t;                                     //诗人技能cd及是否释放技能判定
   ...
   while(1)
    ...
    {//黑魔初始化
        ...
        printf("黑魔法师天赋发动，黑魔纹完成度%d层",heimowen);
        hitor=0;//被打判定清空
    }                  
    {//白魔初始化
        bm_atk_2=0;                                   //攻击倍率初始化
        ...
        printf("\n吟游诗人\n");shiren(sr_cd_zz,sr_buff,sr_debuff,sr_atk_zz,sr_use_zz);
        {
            ...
            {   if(sr_sr_buff>0){sr_sr_buff--;sr_atk*=2;}//如果buff存在伤害翻倍
                ...
                else
                {
                    ...
                    printf("吟游诗人造成了%d的伤害\n",(int)(sr_atk*sr_atk_rank-enemy_real_def));  //诗人未破防伤害
                }
            } 
            if(sr_buff_rank!=0)
            {
    ...
    boss_use=rand()%3+1;//boss随机使用技能
    if(boss_use==1)//一技能单体伤害
    {
        printf("泰坦对前方发动冲锋\n");
        if(dead_bm==0) {maxfind=3;max=find_bm;}         //按仇恨索敌
        if(dead_hm==0)if(find_hm>=max){maxfind=2;}
        if(dead_zs==0)if(find_zs>=max){maxfind=1;}
        if(dead_sr==0)if(find_sr>max){maxfind=4;}
        switch (maxfind)
        {
        case 1:printf("冲向了战士，造成了%d的伤害！\n",(int)(enemy_atk*2.2-real_zs_def));
